function DofManager = dofSetByMaterial(DofManager, dofs,varargin)
%DOFSETBYMATERIAL allows the user to set dofs, continuousness of each field
...
%
%Function signature:
%    DofManager = dofSetByMaterial(DofManager, dofs, continuousness, fieldNames, materialName);
%Example:
%    DofManager = dofSetByMaterial(DofManager, [3,1], {'con', 'dis'}, {'disp', 'pressure'}, 'Iron');
%    for an incompressible problem with discontinuous pressure field
%  or
%    DofManager = dofSetByMaterial(DofManager, [3,1], {'con', 'con'}, {'disp', 'pressure'}, 'Iron');
...
%    DofManager = dofSetByMaterial(DofManager, 3);
%    DofManager = dofSetByMaterial(DofManager, dofs, {'con'}, {''}, Whatever_DM_materialBlock_name_is);   
%
...
%output: DofManager with dofs, continuousness and fieldNames set for a DM material
  ...
  if nargin == 2
      if DofManager.numMaterials > 1
          ...
          fprintf(2,'dofSetByMaterial(DofManager, dofs, continuousness, fieldNames, materialName);\n')
          fprintf(2,'dofSetByMaterial(DofManager, [3,1], {''con'', ''dis''}, {''disp'', ''pressure''}, ''Iron'');\n')
          error('Error! Wrong usage of dofSetByMaterial()!')
      end
      if(size(dofs,2) > 1)
      ...
      DofManager = dofSetByMaterial_internal(DofManager, dofs, {'con'}, {''}, char(keys(DofManager.materials)));
  end
  if nargin > 2
      continuousness = varargin{1};
      fieldNames = varargin{2};
      materialName = varargin{3};
      DofManager = dofSetByMaterial_internal(DofManager, dofs, continuousness, fieldNames, materialName);
  end
  if nargin >5
...
function DofManager = dofSetByMaterial_internal(DofManager, dofs, continuousness, fieldNames, materialName)
